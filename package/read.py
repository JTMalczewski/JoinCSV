...
def splitRow(rows: str) -> list:
    ...
    for i in range(len(rows)):
        ...
        separated[-1] = separated[-1][0:-1]             #deleting the line brake symbol
...
def findMatchingRows(matrix_one: list, matrix_two: list, join_by: str) -> list:
    ...
    key_index_one = np.where(matrix_one[0] == join_by)  #search for the index of the common column in both data files 
    ...
    #iterating by every line in a common column in the first data file and searching for the same values in the second data file 
    for record_one in range(matrix_lenght):
        ...
        if record_two[0].size != 0:                     #saving the index of common value for both data files
            matching_index_one = record_one 
            matching_index_two = record_two[0][0]
        elif record_two[0].size == 0:                   #saving the index of not common valies, empty string is used as a mark
            matching_index_one = record_one
            matching_index_two = ''
